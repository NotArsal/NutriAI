...
class PatientInput(BaseModel):
    ...
    daily_caloric:      int   = Field(..., ge=800, le=4000, examples=[2200],  description="Daily caloric intake in kcal")
    cholesterol:        float = Field(..., ge=100, le=350,  examples=[210.0], description="Total cholesterol in mg/dL")
    blood_pressure:     int   = Field(..., ge=80,  le=200,  examples=[135],   description="Systolic blood pressure in mmHg")
    glucose:            float = Field(..., ge=60,  le=300,  examples=[150.0], description="Fasting glucose in mg/dL")
    weekly_exercise:    float = Field(default=3.0, ge=0, le=20,              description="Weekly exercise hours")
